...
- Summarise at each park multiple attributes that are important for recreation/leisure.
...
```{r, echo=FALSE, fig.align = "center", out.width="100%", fig.cap = paste0("Screenshot: Supply of park area to building residents in Singapore based on OSM data (2020). The color palette is binned according to the quantile values.")}
...
```{r summarise supply of park area per census block, message = FALSE, warning = FALSE}
...
The following figure provides a visual comparison between the provision of park area per capita summarised per census block (subzone), based on conventional metrics (left panel) and derived from each residential building (right panel). The interactive map is viewable online at https://ecological-cities.github.io/home2park/articles/website/metric-comparisons.html.

```{r, echo=FALSE, fig.align = "center", out.width="100%", fig.cap = paste0("Screenshot: Provision of park area per census block (subzone) in Singapore based on OSM data (2020), as measured by the total park area per capita (left panel); and by the supply of park area per capita within residential buildings (right panel). The color palettes are binned according to the quantile values.")}
