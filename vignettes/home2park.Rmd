...
- Redistribute coarse-scale population data (e.g. per census unit) into residential buildings, also known as 'dasymetric mapping'.
- Summarise at each park multiple attributes that are important for recreation.
...
Calculate the per capita park area for each census unit:
...
```{r, echo=FALSE, fig.align = "center", out.width="100%", fig.cap = paste0("Screenshot: Provision of park area per subzone in Singapore based on OSM data (2020), as measured by the total park area per capita (left panel); and by the supply of park area per capita within residential buildings (right panel). The color palettes are binned according to quantile values.")}
